<? include "../config/core.php";
   ...
   // 
	if(isset($_GET['change_staff'])) {
      ...
      if ($id == 'off') $upd = db::query("UPDATE `retail_orders` SET сourier_id = null, order_status = 1 WHERE id = '$order_id'");
   ...
   // cashbox_pay
	if(isset($_GET['store_add'])) {
		$store_name = strip_tags($_POST['store_name']);
		$store_mkr = strip_tags($_POST['store_mkr']);

      $id = (mysqli_fetch_assoc(db::query("SELECT max(id) FROM `store`")))['max(id)'] + 1;
      $ins = db::query("INSERT INTO `store`(`id`, `name_kz`, `mkr`) VALUES ('$id', '$store_name', '$store_mkr')");
      echo $id;
   ...
   // cashbox_plus
	if(isset($_GET['sell_plus'])) {
		$id = strip_tags($_POST['id']);
      $upd1 = db::query("UPDATE `retail_orders_products` SET `sell` = `sell` + 1 WHERE id = '$id';");
      if ($upd1) echo 'yes'; else echo 'none';
      exit();
	}

   // cashbox_minus
	if(isset($_GET['sell_minus'])) {
		$id = strip_tags($_POST['id']);
      $upd1 = db::query("UPDATE `retail_orders_products` SET `sell` = `sell` - 1 WHERE id = '$id';");
      if ($upd1) echo 'yes'; else echo 'none';
      exit();
   ...
   // cashbox_plus
	if(isset($_GET['cancel_plus'])) {
		$id = strip_tags($_POST['id']);
      $upd1 = db::query("UPDATE `retail_orders_products` SET `cancel` = `cancel` + 1 WHERE id = '$id';");
      if ($upd1) echo 'yes'; else echo 'none';
      exit();
	}

   // cashbox_minus
	if(isset($_GET['cancel_minus'])) {
		$id = strip_tags($_POST['id']);
      $upd1 = db::query("UPDATE `retail_orders_products` SET `cancel` = `cancel` - 1 WHERE id = '$id';");
      if ($upd1) echo 'yes'; else echo 'none';
      exit();
   ...
   // cashbox_plus
	if(isset($_GET['cashbox_plus'])) {
      ...
      if ($id) $upd1 = db::query("UPDATE `retail_orders_products` SET `quantity` = `quantity` + 1 WHERE id = '$id';");
      else $ins = db::query("INSERT INTO `retail_orders_products`(`order_id`, `product_id`, `price`, `quantity`) VALUES ('$cid', '$pid', 1000, 1)");
      echo 'yes';
